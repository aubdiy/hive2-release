...
public class SerDeEncodedDataReader extends CallableWithNdc<Void>
  ...
  public void startReadSplitFromFile(
    ...
    try {
      ...
      EncodingWriter writer = VectorDeserializeOrcWriter.create(sourceInputFormat, sourceSerDe,
          parts, daemonConf, jobConf, split.getPath(), originalOi, splitColumnIds, splitIncludes);
  ...
  // TODO: these interfaces are ugly. The two implementations are so far apart feature-wise
  //       after all the perf changes that we might was well hardcode them separately.
  static abstract class EncodingWriter {
    protected WriterImpl orcWriter;
    protected CacheWriter cacheWriter;
    protected final StructObjectInspector sourceOi;

    public EncodingWriter(StructObjectInspector sourceOi) {
      this.sourceOi = sourceOi;
    }


    public void init(CacheWriter cacheWriter, Configuration conf) throws IOException {
      this.orcWriter = new WriterImpl(cacheWriter, null, createOrcWriterOptions(sourceOi, conf));
  ...
  static class DeserializerOrcWriter extends EncodingWriter {
    ...
    public DeserializerOrcWriter(Deserializer sourceSerDe, StructObjectInspector sourceOi) {
      super(sourceOi);
      this.sourceSerDe = sourceSerDe;
    }
  ...
  static WriterOptions createOrcWriterOptions(
      ObjectInspector sourceOi, Configuration conf) throws IOException {
